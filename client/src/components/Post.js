...
import { addComment, deletePost, editPost, getAllPosts, likeOrUnlikePost } from "../redux/actions/postActions";
...
function Post({ post, postInProfilePage }) {
  ...
  return (
    <div className="bs1 p-2 mt-3">
      <div className="d-flex justify-content-between align-items-center">
        <div className="d-flex align-items-center">
          {post.user.profilePicUrl == "" ? (
            <span className="profilepic1 d-flex align-items-center">{post.user.username[0]}</span>
          ) : (
      ...
      <div
        className={
          postInProfilePage ? "d-flex align-items-center justify-content-between" : "d-flex align-items-center"
        }>
            ...
            {post.comments.map(comment => {
              ...
              return (
                <div className="d-flex align-items-center m-1 p-1 justify-content-between">
                  <div className="d-flex align-items-center ">
                    {user.profilePicUrl == "" ? (
                      <span className="profilepic1 d-flex align-items-center">{user.username[0]}</span>
                    ) : (
                      <img src={post.user.profilePicUrl} height="35" width="35" style={{ borderRadius: "50%" }} />
                    )}
                    <Link className="ml-1" style={{ fontSize: 15, marginLeft: 10 }}>
                      {user.username}
                    </Link>
